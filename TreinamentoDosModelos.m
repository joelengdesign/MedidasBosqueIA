...
for i=1:16
    xTrain{i} = DataTrain{i}.Tabela_Atenuacao_Janelada{:,[14 4:5 12]};
    xTrain{i}(:,1:end-1) = (xTrain{i}(:,1:end-1) - min(xTrain{i}(:,1:end-1), [], 1)) ./ (max(xTrain{i}(:,1:end-1), [], 1) - min(xTrain{i}(:,1:end-1), [], 1));

    xTest{i} = DataTest{i}.Tabela_Atenuacao_Janelada{:,[14 4:5 12]};
    xTest{i}(:,1:end-1) = (xTest{i}(:,1:end-1) - min(xTest{i}(:,1:end-1), [], 1)) ./ (max(xTest{i}(:,1:end-1), [], 1) - min(xTest{i}(:,1:end-1), [], 1));

    xValid{i} = DataValid{i}.Tabela_Atenuacao_Janelada{:,[14 4:5 12]};
    xValid{i}(:,1:end-1) = (xValid{i}(:,1:end-1) - min(xValid{i}(:,1:end-1), [], 1)) ./ (max(xValid{i}(:,1:end-1), [], 1) - min(xValid{i}(:,1:end-1), [], 1));
end
%% Modelo 1 - MLP
...
dadosMLP = PSO_MLP(xTrain, xValid, xTest, num_particles, max_epochs)
